...
func GetJSONPieceTest() {
	...
	check(err)
	loc := "1:owner:login"
	luaAndHelpers.GetJSONPiece(string(data), loc) //should return "deef0000dragon1"
}
...
func LuaLimitedEnvironmentPassedInStringTest() {
	...
	var runString = string(dat)
	luaAndHelpers.RunLuaCode(runString)
...
func JSONParseTest(){
	...
	out1:= luaAndHelpers.GetJSONPiece(runString, searchString1)
	out2:= luaAndHelpers.GetJSONPiece(runString, searchString2)
